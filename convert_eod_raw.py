...
def convertEodRaw(eod_file, raw_dir, max_lines=0):
  ...
  with open(eod_file, 'r') as fp:
    while True:
      ...
      num_lines += 1
      if num_lines % PRINT_INTERVAL == 0:
        logging.info('%d lines done' % num_lines)
      if max_lines > 0 and num_lines >= max_lines:
        logging.info('reached debug limit of %d' % max_lines)
        break
  if output_fp is not None:
    output_fp.close()
  print 'processed %d tickers' % len(ticker_date)
